...
router.get('/:username', jwtAuth, (req, res) => {
  ReadingList.find({ username: req.params.username }).then((comics) => {
    const sortedComics = comics.sort((a, b) => b.created - a.created);
    res.json({ data: sortedComics });
  });
});
...
router.put('/:id', [jsonParser, jwtAuth], (req, res) => {
  ...
  ReadingList.findByIdAndUpdate(req.params.id, { $set: updated }, { new: true })
    .then(() => {
      res.status(204).end();
    })
    .catch(() => {
...
router.delete('/:id', jwtAuth, (req, res) => {
  ReadingList.findByIdAndRemove(req.params.id)
    .then(() => {
